...
@foreach ($sectionss as $sec)
<?php
  $students = \App\Models\StudentInfo::where('campusid', Auth::user()->campusid)->where('admissioninclass', $request->class)
  ->where('admissioninsection', $sec->admissioninsection)->where('status', '<>', 'Slc')->where('status', '<>', 'Matriculate')->get();
?>
<div class="row pagebreak" id>
  ...
  <div class="col-md-10 text-center">
    ...
    @endif
    <?PHP
      $classseees = \App\Models\addClass::where('campusid', Auth::user()->campusid)->where('C_id', $students[0]->admissioninclass)->first();
      $seccctions = \App\Models\addsection::where('campusid', Auth::user()->campusid)->where('Sec_ID', $students[0]->admissioninsection)->first();
      $counter = 1;
    ?>
    <h3>{{ $classseees->ClassName . '(' . $seccctions->SectionName . ')' }}  - {{ date('M, Y', strtotime($request->attendancemonth)) }}</h3>
  </div>
</div>
<div class="row">
  <div class="col-md-12">
    ...
    <table class="table table-bordered">
      <thead>
        ...
        <tr>
          ...
          <th>B#</th>
          <?php 
          $date = $request->attendancemonth . '-01';
          $end = $request->attendancemonth . '-' . date('t', strtotime($date)); //get end date of month
        ?>
