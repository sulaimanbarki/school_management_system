...
<div class="row">
  <div class="col-md-12">
    <?php
      ...
      $expenseheads = \App\Models\ExpenseHead::where('campusid', Auth::user()->campusid)->get();
      $accounts = \App\Models\Account::where('campusid', Auth::user()->campusid)->get();
    ?>
    ...
    <form id="ReportForm" action="/admin" method="POST">
      ...
      <div class="row  pt-1 pb-1">
        <div class="col-md-3">
          ...
          <input type="date" value="<?php echo date('Y-m-d'); ?>" required name="fromdate" id="fromdate"
            class="form-control form-control-sm">
        </div>
        <div class="col-md-3">
          <label for="ToDate" class="form-label"><b>To Date</b></label>
          <input type="date" class="form-control form-control-sm" required value="<?php echo date('Y-m-d'); ?>" name="todate"
            id="todate">
        </div>
        <div class="col-md-3">
          <label for="expensehead" class="form-label"><b>Expense Head</b></label>
          <select class="form-control form-control-sm mb-2" id="expensehead" name="expensehead" placeholder="">
            <option value="">Select Head</option>
            @foreach ($expenseheads as $head)
                <option value="{{ $head->id }}">{{ $head->expense_head }}</option>
            @endforeach
          </select>
        </div>
        <div class="col-md-3">
          <label for="accountid" class="form-label"><b>Accounts</b></label>
          <select class="form-control form-control-sm mb-2" id="accountid" name="accountid" placeholder="">
...
<div class="row">
  <div class="col-md-3">
      <button formtarget='_blank' formaction="/admin/ExpenseReport" type="submit" form="ReportForm"
        class="btn btn-sm btn-primary btn-block" onclick="DisableFormBothAttribute(event)">
        Expense Report</button>

      <button formtarget='_blank' formaction="/admin/AccountReport"
        onclick="DisableFormBothAttribute(event)" type="submit" form="ReportForm"
        class="btn btn-sm btn-primary btn-block">Accounts Report</button>

      <button formtarget='_blank' formaction="/admin/NetProfit"
        onclick="DisableFormSectionAttribute(event)" type="submit" form="ReportForm"
        class="btn btn-sm btn-primary btn-block">Net Profit</button>

      <button formtarget='_blank' formaction="/admin/ExpensesGroupBy" type="submit" form="ReportForm"
        class="btn btn-sm btn-primary btn-block">Expenses Sum</button>

      <button formtarget='_blank' formaction="/admin/AccountsGroupBy" type="submit" form="ReportForm"
        class="btn btn-sm btn-primary btn-block">Accounts Sum</button>
